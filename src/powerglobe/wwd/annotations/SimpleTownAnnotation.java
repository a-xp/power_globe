...
@XmlRootElement
public class SimpleTownAnnotation extends AbstractAnnotation{
	
	{
		state.put("text", "Неизвестный город");
	}
	
	public static String getName() {
		return "Текст - название города";
	}
	...
	public GlobeAnnotation getAnnotation(Position position) {
		...
		return new GlobeAnnotation(state.get("text"), Position.fromDegrees(position.latitude.degrees, position.longitude.degrees, 0), townAttr);
		
	}

	public static Map<String, AbstractAnnotationProp> getControls(){
		Map<String, AbstractAnnotationProp> result = new HashMap<>();
		result.put("text", new SingleLine("Название города"));
		return result;
	}
